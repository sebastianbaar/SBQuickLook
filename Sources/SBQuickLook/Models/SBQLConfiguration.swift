...
public struct SBQLConfiguration {
    ...
    ///
    /// - Parameters:
    ///   - session: Optional `URLSession`. Overrides the `URLSession` used by the download task.
    ///   Default: `URLSession.shared`
    ///   - localFileDir: Optional local directory `URL`. Overrides the local directory `URL` used by the download task.
    ///   Default: `FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)`
